package com.hxzy.controller.from;
...
import com.hxzy.dto.ShopRegisterSMSDTO;
...
public class FrontSMSController extends BaseController {
    ...
    public Result registerSMS(@PathVariable(value = "phone") String phone) {
        ...
        String redisKey= RedisConstant.getRedisKey(RedisConstant.SMS_SHOP_REGISTER_REDIS_KEY,uuid);
        if(this.redisTemplate.hasKey(redisKey)){
            Result r=Result.build(AckCode.SMS_SEND_COLD_TIME_FORMAT);
            r.setMsg( StrUtil.format( r.getMsg(),smsProperties.getExpired()));
            return r;
        }

        // 验证号码是否已经注册过了
        boolean exists = this.merchandiserService.existsLoginName(phone);
        if(exists){
...
//            }
            ...
            if (status.equalsIgnoreCase("OK")) {
                ...
                data.put("expired", this.smsProperties.getExpired()*60);

                return Result.okHasData(data);
            } else {
    ...
    @ApiOperation(value = "验证商家注册短信")
    @PostMapping(value = "/register/validate")
    public Result registerShopSMSValidator(@RequestBody @Valid ShopRegisterSMSDTO dto){
        String  uuid=SecureUtil.md5(dto.getPhone());
        String redisKey=RedisConstant.getRedisKey(RedisConstant.SMS_SHOP_REGISTER_REDIS_KEY,uuid);
        Object redisValue=this.redisTemplate.opsForValue().get(redisKey);

        if(null==redisValue){
            Result r=Result.build(AckCode.SMS_CODE_OVERTIME);
            return r;
        }

        if(!dto.getCode().equalsIgnoreCase(redisValue.toString())){
